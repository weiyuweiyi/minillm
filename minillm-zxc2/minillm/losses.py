...
class Loss():
    ...
    # policy gradient 的 advantage 部分
    ...
    # single 损失（正则损失，减轻 long norm 损失的误差）
    ...
    def pt_loss(self, batch, logits):
        ...
        if self.trainer.teacher_model is not None and self.args.kd_ratio is not None:
            ...
            # else:
            #     # 自实现的前向 KD 损失，KD Baseline，https://github.com/microsoft/LMOps/issues/116
            ...
            # TODO: 多教师
            loss_list = []
            for teacher_model in self.trainer.teacher_model:
                with torch.no_grad():
                    teacher_outputs = teacher_model(**model_batch, return_dict=True, use_cache=False)
                    teacher_logits = teacher_outputs.logits
                if self.args.model_parallel:
                    distil_losses = mpu.parallel_soft_cross_entropy_loss(logits.float(), teacher_logits.float())
                    distil_losses = distil_losses.view(-1)
                ...
                # print("Expanded Loss mask shape: ", loss_mask.unsqueeze(-1).shape)
                # Calculate L1 distance between teacher and student logits and add to distil_loss
                # l1_loss = torch.sum(torch.abs(logits - teacher_logits) * loss_mask.unsqueeze(-1), dim=[0, 1]) / torch.sum(loss_mask)
                # distil_loss += l1_loss  # Add the L1 loss to the distillation loss
                student_probs = F.softmax(logits, dim=-1)
                # l1_loss = F.smooth_l1_loss(student_probs, teacher_probs, reduction='none')
                # l1_loss = (l1_loss * loss_mask.unsqueeze(-1)).sum() / loss_mask.sum()
